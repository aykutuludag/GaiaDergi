...
public class FragmentHome extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 21) {
            ...
            window.setStatusBarColor(ContextCompat.getColor(getActivity(), R.color.colorHomeDark));
        }
        ActionBar bar = ((MainActivity) getActivity()).getSupportActionBar();
        assert bar != null;
        bar.setBackgroundDrawable(new ColorDrawable(ContextCompat.getColor(getActivity(), R.color.colorHomePrimary)));

        pb = v.findViewById(R.id.progressBar1);
        pb.getIndeterminateDrawable().setColorFilter(ContextCompat.getColor(getActivity(), R.color.colorHomePrimary), PorterDuff.Mode.SRC_IN);
    ...
    private class ExecuteNetworkOperation extends AsyncTask<String, Void, ArrayList<RssItem>> {
        ...
        protected ArrayList<RssItem> doInBackground(String... params) {
            try {
                url = new URL("https://gaiadergi.com/feed");
