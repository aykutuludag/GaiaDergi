...
public class FragmentEvents extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 21) {
            ...
            window.setStatusBarColor(ContextCompat.getColor(getActivity(), R.color.colorEventsDark));
        }
        ActionBar bar = ((MainActivity) getActivity()).getSupportActionBar();
        bar.setBackgroundDrawable(new ColorDrawable(ContextCompat.getColor(getActivity(), R.color.colorEventsPrimary)));

        pb = v.findViewById(R.id.progressBar1);
        pb.getIndeterminateDrawable().setColorFilter(ContextCompat.getColor(getActivity(), R.color.colorEventsPrimary), PorterDuff.Mode.SRC_IN);

        // Analytics
        Tracker t = ((AnalyticsApplication) getActivity().getApplication()).getDefaultTracker();
        t.setScreenName("Duyurular&Etkinlikler");
    ...
    private class ExecuteNetworkOperation extends AsyncTask<String, Void, ArrayList<RssItem>> {
        ...
        protected ArrayList<RssItem> doInBackground(String... params) {
            try {
                url = new URL("https://gaiadergi.com/kategori/duyurular-etkinlikler/feed/");
