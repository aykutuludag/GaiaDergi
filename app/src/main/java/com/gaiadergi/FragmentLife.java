...
public class FragmentLife extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 21) {
            ...
            window.setStatusBarColor(ContextCompat.getColor(getActivity(), R.color.colorLifeDark));
        }
        ActionBar bar = ((MainActivity) getActivity()).getSupportActionBar();
        assert bar != null;
        bar.setBackgroundDrawable(new ColorDrawable(ContextCompat.getColor(getActivity(), R.color.colorLifePrimary)));

        pb = v.findViewById(R.id.progressBar1);
        pb.getIndeterminateDrawable().setColorFilter(ContextCompat.getColor(getActivity(), R.color.colorLifePrimary), PorterDuff.Mode.SRC_IN);

        // Analytics
        Tracker t = ((AnalyticsApplication) getActivity().getApplication()).getDefaultTracker();
        t.setScreenName("Yaşam");
    ...
    private class ExecuteNetworkOperation extends AsyncTask<String, Void, ArrayList<RssItem>> {
        ...
        protected ArrayList<RssItem> doInBackground(String... params) {
            try {
                url = new URL("https://gaiadergi.com/kategori/yasam/feed");
